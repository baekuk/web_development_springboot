...
/*
    New project 생성 시 주의점:
        ...
        2. DLS -> Groovy로 설정
        3. name = ArtifactID와 동일
        4. build.gradle 설정을 복사하는데, 복사 후에 -> sync를 꼭 눌러주셔야 합니다.
            -> SpringBootDeveloperApplication에서 @SpringBootApplication 애너테이션에 빨간줄이면
                새로고침 안했을 확률이 거의 90%
...
/*
    1. Spring vs Spring Boot
        ...
        Enterprise Application : 대규모의 복잡한 데이터를 관리하는 어플리케이션
        ...
        SpringBoot의 등장
            ...
            특징 :
                1) 톰캣, 제티, 언더토우 같은 웹 애플리케이션 서버가 내장되어 있어 따로 설치하지
                    않아도 독립적으로 실행 가능
             ...
             차이점 :
                ...
                2) 내장 WAS 유뮤 -> 스프링 애플리케이션은 일반적으로 톰캣과 같은 WAS에서 배포
    ...
    2. 스프링 컨셉
        1) 제어의 역전(IoC)
            Inversion of Control : 다른 객체를 직접 생성하거나 제어하는 것이 아니라
                ...
                ex) 클래서 A에서 클래스 B의 객체를 생성한다고 가정했을 때의 자바 코드
                public class A {
                    B b = new B();
                }
        ...
        5) 이식 가능한 서비스 추상화(PSA)
            Portable Service Abstraction : 스프링에서 제공하는 다양한 기술들을 '추상화' 해
                ...
                어떤 기술로 사용하든 일관된 방식으로 데이터베이스에 접근하도록 인터페이스를 지원
                WAS 역시 PSA의 예시 중 하나로, 어떤 서버를 사용하더라도(톰캣, 언  더토우, 네티 등)
                코드를 동일하게 작성 가능
