...
-- 1. SELECT : 보여달라 / 조회해라 
select "Hello, SQL!";
-- select : 보여달라 / Hello, SQL!이라는 str을 보여달라
...
select * from products limit 3; -- 상위 3개를 가져옴
-- 이렇게 *를 사용해서 전체 정보를 조회하는 것을 full scan이라고 하는데, 빈도가 높지 않음.
-- 물론 시험에도 자주 나오지 않습니다. 

-- 개수 제한을 걸기 위한 LIMIT와 특정 컬럼을 조회하는 형태로 수업을 하도록 하겠습니다. 

-- SELECT 컬럼명1, 컬럼명2 from 테이블명;
-- 제품 정보 테이블 products에서 제품 아이디(id), 제품명(name) 컬럼만 출력합니다.
